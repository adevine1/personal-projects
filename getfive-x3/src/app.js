...
function clearScoreHistory() {
  let check = confirm(
    "Are you sure that you want to reset the entire score history?"
  );
  if (check == true) {
    let scoreObject = {};
...
function calculateSubtotals() {
  ...
  if (topcount >= 63) bonusTable.rows[1].cells[1].innerText = 35;

  if (bonusTable2.rows[0].cells[1].innerText !== "")
    bonus2Count = parseInt(bonusTable2.rows[0].cells[1].innerText);
  else bonus2Count = 0;

  grandTotalTable.rows[0].cells[1].innerText =
    topcount + lowercount + bonus2Count;
}
...
function submitValueBottom(element) {
  ...
  if (a !== "") {
    bottomTable.rows[element.rowIndex].cells[1].innerText = a.slice(
      1,
      a.length
    );
...
function submitValueBonusChips(element) {
  ...
  if (a !== "") {
    if (bonusTable2.rows[0].cells[1].innerText !== "") {
      bonusTable2.rows[0].cells[1].innerText =
        parseInt(bonusTable2.rows[0].cells[1].innerText) + 100;
    } else {
...
function diceClick(element) {
  ...
  for (let x = 0; x < 5; x++) {
    if (element.id === diceArray[x]) heldDice[x] = !heldDice[x];
  }
...
function randomizeUnheldDice() {
  ...
  for (
    var individualDiceRoll = 0;
    individualDiceRoll < 6;
    individualDiceRoll++
  ) {
  ...
  for (
    var displayRolledDice = 0;
    displayRolledDice < currentDiceRoll.length;
    displayRolledDice++
  )
    if (heldDice[displayRolledDice] === false)
      document.getElementById(diceArray[displayRolledDice]).innerHTML =
        currentDiceRoll[displayRolledDice];
}
...
function determineScoringOptions() {
  ...
  for (let x = 0; x < 5; x++) {
    for (let y = 0; y < 6; y++) {
      if (
        document.getElementById(diceArray[x]).innerHTML === diceVariations[y] ||
...
function calculateCurrentScore(diceArray) {
  ...
  if (bottomTable.rows[2].cells[1].innerText === "") {
    if (
      (diceArray.indexOf(3) !== -1 && diceArray.indexOf(2) !== -1) ||
      diceArray.indexOf(5) !== -1
    )
  ...
  if (bottomTable.rows[4].cells[1].innerText === "") {
    ...
    else bottomTable.rows[4].cells[2].innerHTML = "←" + 0;
...
function endgameCheck(finalScore) {
  ...
  if (
    topTable.rows[0].cells[1].innerText !== "" &&
    ...
    button.innerText =
      "submit score of " + finalScore + " to the historical record";
    button2.disabled = false;
    button2.style.color = "yellow";
    button2.style.backgroundColor = "#173b67";
    button2.innerText = "or click here to start new game";
  }
}
...
function rollTheDice() {
  ...
  if (document.getElementById("rollerbutton").innerText !== "Roll") {
    result = prompt(
      "Please enter your name to submit to High Score List",
      "[enter name here]"
    );

    if (result !== null) {
      let endscore = document.getElementById("grandtotal").rows[0].cells[1]
        .innerText;
      alert(
        result.toUpperCase() +
